        ...
        while thread.is_alive and delta <= ttl_secs:
            try:
                ...
                if container.status != 'running':
                    ...
                    self.log.info(
                        'waiting for container to start running; %ss elapsed; ttl_secs (before job teardown)=%ss' % (
                        delta, ttl_secs))
                    sleep(2)
                else:
                    if self.params["server_mode"]:
                ...
                self.log.info(e)
                delta = int((time_now() - time_start).total_seconds())
                self.log.info('container not found; retrying; %ss elapsed; ttl_secs (before job teardown)=%ss' % (
                delta, ttl_secs))
                sleep(2)
                continue
            except Exception as e:
...
def main():
        ...
        if scavenger_mode_active and overhead:
            ...
            if job_type == "atq":
                ...
                if 'rescheduled' in msg and msg['rescheduled']:
                    ...
                    terminate['reason'] = 'job run reschedule failed (scavenger mode is active); reason: %s' % msg[
                        'message']
    ...
    print("experiment config is ***", experiment_config_dict)
